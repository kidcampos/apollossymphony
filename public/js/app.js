...
class App{
                  ...
                  <img src="${this.pianopreview[index].image}">
                    ...
                    <div class="section" >
                      <div class="col s12 m12 l6" id="textBox">
                            ...
                            <p id="price">Php ${this.pianopreview[index].price}</p>
                            <a href="#modal1" data-target="modal1" class="waves-effect waves-light btn modal-trigger" id="modalbtn" onclick="component.shoppingListAdd('${this.pianopreview[index].artist}','${this.pianopreview[index].title}','${this.pianopreview[index].instrument}','${this.pianopreview[index].price}')"><i class="material-icons right" id="shop_cart">shopping_cart</i>Add to Cart</a>
                  ...
                  <img src="${this.violinpreview[index].image}">
                    ...
                    <div class="section" >
                      <div class="col s12 m12 l6" id="textBox">
                            ...
                            <p id="price">Php ${this.violinpreview[index].price}</p>
                            <a href="#modal1" data-target="modal1" class="waves-effect waves-light btn modal-trigger" id="modalbtn" onclick="component.shoppingListAdd('${this.violinpreview[index].artist}','${this.violinpreview[index].title}','${this.violinpreview[index].instrument}','${this.violinpreview[index].price}')"><i class="material-icons right" id="shop_cart">shopping_cart</i>Add to Cart</a>
  ...
  searchCartSong(){
    ...
    for(let i=0;i<this.cart.length;i++){
      if(this.cart[i].artist.toLowerCase().includes(txtSearchCartSong.value.toLowerCase())||this.cart[i].title.toLowerCase().includes(txtSearchCartSong.value.toLowerCase())||this.cart[i].instrument.toLowerCase().includes(txtSearchCartSong.value.toLowerCase())){
        html += `<tr>
        <td>${this.cart[i].artist}</td>
        <td>${this.cart[i].title}</td>
        <td>${this.cart[i].instrument}</td>
        <td>${this.cart[i].price}</td>
        <td><a href="#!" id="action" onclick="component.deleteCart(${i})"><i class="material-icons">delete</i></a></td>
        </tr>`; 
      }
    }
  ...
  shoppingListAdd(artist, title, instrument, price){
    let shopcart = {"artist":artist,"title":title, "instrument":instrument,"price":price};
  ...
  cartInventory(){ 
      ...
      for(let index=0; index<this.cart.length; index++){
       html +=`
        <tr id="${idName+index}">
          ...
          <td><a href="#!" id="action" onclick="component.sheetUpdateInput(${index})"><i class="material-icons">mode_edit</i></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#!" id="action" onclick="component.deleteCart(${index})"><i class="material-icons">delete</i></a></td>
        </tr>
      `;
      num = num + parseFloat(this.cart[index].price);
      this.cartTotal(num);
      }
      if(this.cart.length == 0){
        ...
        html +=`
        <tr>
          <td colspan="5">You have not purchased anything</td>
        </tr>
      `;
  ...
  updateSheet(key){
    ...
    if(!var_price.value){
      let sheet = {"artist":var_artist.value,"title":var_title.value,"instrument":var_instrument.value,"price":this.cart[key].price};
      this.cart[key] = sheet;
    }
    else{
      let product=var_price.value * 50.55;
      let dec = product.toFixed(2);
      let sheet = {"artist":var_artist.value,"title":var_title.value,"instrument":var_instrument.value,"price":dec};
      this.cart[key] = sheet;
    }
  ...
  sheetUpdateInput(val){
    ...
    let html = `
      <tr>
       ...
       <td><input id="updatePrice" type="text" placeholder="Enter number of pages..." /><td>
  ...
  createSheet(){
    ...
    if((var_artist.value && var_artist)&&(var_title.value&& var_title) && (var_instrument.value&&var_instrument)){
    let piece = {"artist":var_artist.value,"title":var_title.value,"instrument":var_instrument.value,"price":"50.55"};
	...
	indexPage(){
      ...
      <div id="shoppingcart">
        ...
        <div class="section">
          ...
          <div class="container">
            <p id="cart-text">If you didn&apos;t find what you&apos;re looking for, preorder now by entering the title of the score below and its artist/composer. There will be a down-payment of preordering which the price is Php 50.55 per page.</p>
            <form>
             <input id="txtAddArtist" type="text" placeholder="Enter artist here..." required/>
             <input id="txtAddTitle" type="text" placeholder="Enter title here..." required/>
             <input id="txtAddInstrument" type="text" placeholder="Enter instrument here..." required/>
          ...
          <div class="row">
            ...
            <div class="col l3 s12">
              ...
              <ul id="footertext2"> 
                ...
                <li><a href="#" onclick="component.violinSheetPage()">Violin</a></li>
              </ul>
            </div>
          </div>
        </div>
